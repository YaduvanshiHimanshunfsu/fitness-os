...
drop policy if exists "Public Access exercise_images" on storage.objects;
drop policy if exists "Admin Upload Access exercise_images" on storage.objects;
drop policy if exists "Admin Delete Access exercise_images" on storage.objects;

-- 3. Create Policy: Anyone can view exercise images
create policy "Public Access exercise_images"
on storage.objects for select
using ( bucket_id = 'exercise_images' );

-- 4. Create Policy: Only Admin can insert/upload
create policy "Admin Upload Access exercise_images"
...
create policy "Admin Delete Access exercise_images"
