...
export default async function ProfilePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-8 pb-24">
      {/* Page Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-black text-zinc-900 dark:text-white tracking-tight">Profile</h1>
        <p className="text-zinc-500 text-sm mt-1">Manage your personal information and account settings</p>
      </div>

      {/* Profile Card */}
      <div className="bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 mb-6 shadow-xl">
        {/* Avatar + Name Row */}
        <div className="flex items-center gap-6 mb-8">
          ...
          <div>
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">{profile?.name || 'Fitness Athlete'}</h2>
        ...
        <form action={handleUpdateProfile} className="space-y-5">
          <div>
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl px-4 py-3 pr-10 text-zinc-900 dark:text-white placeholder:text-zinc-600 focus:outline-none focus:border-[#FF4500]/60 focus:ring-1 focus:ring-[#FF4500]/30 transition-all"
              />
          ...
          <div>
            ...
            <input
              ...
              value={user.email || ''}
              readOnly
              className="w-full bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800/60 rounded-xl px-4 py-3 text-zinc-500 cursor-not-allowed"
            />
          </div>
          ...
          <div className="pt-2">
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-[#FF4500] to-[#E03C00] hover:from-[#E03C00] hover:to-[#CC3500] text-zinc-900 dark:text-white font-black py-3.5 rounded-xl uppercase tracking-widest text-sm transition-all hover:shadow-[0_0_30px_rgba(255,69,0,0.4)]"
            >
              Save Profile
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 flex flex-col items-center justify-center text-center hover:border-zinc-300 dark:border-zinc-700 transition-colors">
          <Activity className="w-6 h-6 text-[#10B981] mb-2" />
          <span className="text-2xl font-bold text-zinc-900 dark:text-white leading-none mb-1">{profile?.total_workouts || 0}</span>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 flex flex-col items-center justify-center text-center hover:border-zinc-300 dark:border-zinc-700 transition-colors">
          ...
          <span className="text-2xl font-bold text-zinc-900 dark:text-white leading-none mb-1">{profile?.xp_total || 0}</span>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 flex flex-col items-center justify-center text-center hover:border-zinc-300 dark:border-zinc-700 transition-colors">
          ...
          <span className="text-2xl font-bold text-zinc-900 dark:text-white leading-none mb-1">{profile?.total_sets || 0}</span>
