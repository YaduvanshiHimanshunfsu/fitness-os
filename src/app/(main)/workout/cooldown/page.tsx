...
export default function CoolDownPage() {
  ...
  const [timeLeft, setTimeLeft] = useState(COOLDOWN_EXERCISES[0]?.durationSeconds || 30);
  const [restTimeLeft, setRestTimeLeft] = useState(15);

  const currentExercise = COOLDOWN_EXERCISES[currentIndex];
  const totalStretchTime = currentExercise?.durationSeconds || 30;
  ...
  const handleNext = () => {
    if (currentIndex < COOLDOWN_EXERCISES.length - 1) {
      setCurrentIndex((prev) => prev + 1);
      setTimeLeft(COOLDOWN_EXERCISES[currentIndex + 1].durationSeconds);
      setRestTimeLeft(15);
          ...
          >
            ...
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <span className="text-[10px] font-mono font-black tracking-widest text-zinc-500 uppercase mt-1">
                Stretch {currentIndex + 1} of {COOLDOWN_EXERCISES.length}
              </span>
            </div>
