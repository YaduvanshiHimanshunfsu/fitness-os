...
export function LevelProgress({ 
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 md:p-8 mb-12 relative overflow-hidden">
      {/* Background Glow */}
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between mb-6 gap-4">
        <div>
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight">
            Level {currentLevel.level} — <span className="text-zinc-400">{currentLevel.name}</span>
          </h2>
        </div>
        <div className="text-left md:text-right">
          <span className="text-2xl font-bold text-zinc-300">{xp} <span className="text-sm font-medium text-zinc-500 uppercase tracking-widest">XP</span></span>
          {xpNeeded > 0 && (
      ...
      <div className="relative z-10 w-full h-4 bg-zinc-950 rounded-full overflow-hidden border border-zinc-800">
        <motion.div 
          initial={{ width: 0 }}
