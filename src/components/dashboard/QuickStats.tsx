...
function StatCard({ label, value }: { label: string, value: number }) {
  return (
    <motion.div variants={fadeUp} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg p-4 text-center flex flex-col justify-center">
      <span className="text-xs text-zinc-500 uppercase tracking-wider mb-1 block">{label}</span>
      <span className="text-xl font-bold text-zinc-800 dark:text-zinc-100">{value}</span>
    </motion.div>
  )
}
