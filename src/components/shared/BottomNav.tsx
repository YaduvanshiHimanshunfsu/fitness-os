...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 z-40 bg-[#0A0A0A]/95 backdrop-blur-md border-t border-[#1F1F1F] md:hidden flex justify-around items-center px-2">
      {ITEMS.map((item) => {
        ...
        return (
          <Link
            ...
            className={`flex flex-col items-center justify-center w-14 h-12 rounded-lg transition-all ${
              isActive ? 'text-[#FF6B35]' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
